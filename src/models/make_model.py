...
def make_model(opt, device, **kwargs):
    ...
    if opt['model'] == '1d_cnn':
        model = Net(features = opt['features'], window = opt['window'], opt = opt, task = 'classification', \
                    invasive = opt['invasive'], multi = opt['multi'], ehr_dim= len(opt['colum_ehr']), use_ehr=opt['use_ehr'])
    elif opt['model'] == 'comb':
        model = Nets(features = opt['features'],  window = opt['window'], opt = opt, task = 'classification',invasive = opt['invasive'],\
